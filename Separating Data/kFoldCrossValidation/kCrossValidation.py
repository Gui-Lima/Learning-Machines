...
class kFoldCrossValidation:
    ...
    def __init__(self, dataset, k, validationColumn, debug=True):
    ...
    def run(self, traningSets=[], avaliationSets=[], stratified=True):
        ...
        if stratified:
            folds = makeRandomFolds(self.k, self.groups)
        else:
            folds = makeFolds(self.groups)
    ...
    def makeRandomFolds(self, seed=42):
        folds = []
        setSize  = round(len(self.groups)/self.k)
        percentagesOfEachGroup = getPercentagesOfEachElement(self.groups)
        groupAndIndex=[]
